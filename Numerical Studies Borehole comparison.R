...
### training data ###
n6 <- 2; n5 <- round(c^gam*n6); n4 <- round(c^(2*gam)*n6); n3 <- round(c^(3*gam)*n6); n2 <- round(c^(4*gam)*n6); n1 <- round(c^(5*gam)*n6);
m1 <- 2.5; m2 <- m1*c; m3 <- m2*c; m4 <- m3*c; m5 <- m4*c; m6 <- m5*c;
d <- 8
eps <- sqrt(.Machine$double.eps)
set.seed(0)
x <- maximinLHS(100, d)
y.test <- matrix(apply(x, 1, output.f, t=0))

rep <- 100
result.borehole.rmse <- result.borehole.crps <- matrix(NA, rep, 6)
colnames(result.borehole.rmse) <- colnames(result.borehole.crps) <- c("DNAmf", "RNAmf", "Cokriging", "NARGP", "Fractional BM", "BM")
beta <- delta <- c()
...
for(i in 1:rep) {
  ...
  if(NARGP) saveRDS(list(X1=X1, X2=X2, X3=X3, Y1=y1, Y2=y2, Y3=y3, Xtest=x, Ytest=y.test), file = "tmp_data.rds")
  ...
  fit.DNAmf.sqex <- DNAmf.sqex(X1, y1, X=rbind(X2, X3, X4, X5, X6), y=rbind(y2, y3, y4, y5, y6),
                               ...
                               t=c(m1,m2,m3,m4,m5,m6), multi.start=20, constant=TRUE)#, max_cor=0.8)
  ...
  pred.fbm <- predict(fit.fbm, x, matrix(rep(0,nrow(x))) )
  ...
  fit.bm <- MuFiMeshGP(rbind(X1, X2, X3, X4, X5, X6),
                       ...
                       rbind(y1, y2, y3, y4, y5, y6),
                       H.known = 0.5)
  pred.bm <- predict(fit.bm, x, matrix(rep(0,nrow(x))) )
  ...
  y1 <- matrix(apply(X1, 1, output.f, t=m2))
  y2 <- matrix(apply(X2, 1, output.f, t=m3))
  y3 <- matrix(apply(X3, 1, output.f, t=m4))
  ...
  ### RMSE ###
  result.borehole.rmse[i,1] <- sqrt(mean((predydiffu.sqex-y.test)^2)) #
  result.borehole.rmse[i,2] <- sqrt(mean((predy.RNAmf-y.test)^2)) #
  result.borehole.rmse[i,3] <- sqrt(mean((pred.muficokm$mean-y.test)^2)) #
  if(NARGP) result.borehole.rmse[i,4] <- py$error
  result.borehole.rmse[i,5] <- sqrt(mean((predyfbm-y.test)^2)) #
  result.borehole.rmse[i,6] <- sqrt(mean((predybm-y.test)^2)) #

  result.borehole.crps[i,1] <- mean(crps(y.test, predydiffu.sqex, predsig2diffu.sqex)) #
  result.borehole.crps[i,2] <- mean(crps(y.test, predy.RNAmf, predsig2.RNAmf)) #
  result.borehole.crps[i,3] <- mean(crps(y.test, pred.muficokm$mean, pred.muficokm$sig2)) #
  if(NARGP) result.borehole.crps[i,4] <- py$crps
  result.borehole.crps[i,5] <- mean(crps(y.test, predyfbm, predsig2fbm)) #
  result.borehole.crps[i,6] <- mean(crps(y.test, predybm, predsig2bm)) #
