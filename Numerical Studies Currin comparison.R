...
set.seed(0)
x <- maximinLHS(100, d)
y.test <- matrix(apply(x, 1, currin, t=0))

rep <- 100
result.currin.rmse <- result.currin.crps <- matrix(NA, rep, 6)
colnames(result.currin.rmse) <- colnames(result.currin.crps) <- c("DNAmf", "RNAmf", "Cokriging", "NARGP", "Fractional BM", "BM")
beta <- delta <- c()
...
for(i in 1:rep) {
  ...
  if(NARGP) saveRDS(list(X1=X3, X2=X4, X3=X5, Y1=y3, Y2=y4, Y3=y5, Xtest=x, Ytest=y.test), file = "tmp_data.rds")
  ...
  X3 <- NestDesignMuFiCokriging$PX
  X4 <- ExtractNestDesign(NestDesignMuFiCokriging,2)
  X5 <- ExtractNestDesign(NestDesignMuFiCokriging,3)
  y3 <- matrix(apply(X3, 1, currin, t=m3))
  y4 <- matrix(apply(X4, 1, currin, t=m4))
  y5 <- matrix(apply(X5, 1, currin, t=m5))
  ...
  ### RMSE ###
  result.currin.rmse[i,1] <- sqrt(mean((predydiffu.sqex-y.test)^2)) #
  result.currin.rmse[i,2] <- sqrt(mean((predy.RNAmf-y.test)^2)) #
  result.currin.rmse[i,3] <- sqrt(mean((pred.muficokm$mean-y.test)^2)) #
  if(NARGP) result.currin.rmse[i,4] <- py$error
  result.currin.rmse[i,5] <- sqrt(mean((predyfbm-y.test)^2)) #
  result.currin.rmse[i,6] <- sqrt(mean((predybm-y.test)^2)) #

  result.currin.crps[i,1] <- mean(crps(y.test, predydiffu.sqex, predsig2diffu.sqex)) #
  result.currin.crps[i,2] <- mean(crps(y.test, predy.RNAmf, predsig2.RNAmf)) #
  result.currin.crps[i,3] <- mean(crps(y.test, pred.muficokm$mean, pred.muficokm$sig2)) #
  if(NARGP) result.currin.crps[i,4] <- py$crps
  result.currin.crps[i,5] <- mean(crps(y.test, predyfbm, predsig2fbm)) #
  result.currin.crps[i,6] <- mean(crps(y.test, predybm, predsig2bm)) #
