...
nonnested_plots <- mapply(function(i, m, mu, sig2, X, y) {
  create_plot(i, m, x, mu, sig2, X, y, add_points = !is.null(X), yylim=1.86)
}, i = 1:6, m = mesh_sizes, mu = mu_list, sig2 = sig2_list, X = X_list, y = y_list, SIMPLIFY = FALSE)

# Arrange plots
figure8 <- annotate_figure(ggarrange(plotlist = nonnested_plots, ncol = 6, nrow = 1) + ylab("y"),
